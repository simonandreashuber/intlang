    ...
    List.iter 
      (
        fun stmt -> 
            match stmt with
              | Let (name, e) -> (
                  let nl = if is_include then Let (basename ^ "." ^ name, vars_add_prefix basename [] e) else Let (name, e) in
                  letacc := nl :: !letacc;
                )
              | Letrec (name, e) -> (
                  let nl = if is_include then Letrec (basename ^ "." ^ name, vars_add_prefix basename [] e) else Letrec (name, e) in
                  letacc := nl :: !letacc;
                )
              | Letrecblk lst -> (
                  let ltuplst = List.map (fun (name, e) -> 
                    if is_include then (basename ^ "." ^ name, vars_add_prefix basename [] e) else (name, e)
                  ) lst in
